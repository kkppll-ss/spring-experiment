...
class Produce_Read_Order_List:
    ...
    def start_up(self, user_num):
        ...
        f = open("User_Records/Participant"+str(self.current_user_num) + "/order_list.txt", "w")
        header = "Pin Height,Recognition_Load,Force_Profile,Electronic_Component,Ask_Last_Num\n"
        f.write(header)
        ...
        for i in range(len(pin_condition_order)):
            ...
            while len(cognitive_load_handness_pairs_list[i]) != 0:
                ...
                while len(fp_ec) != 0:
                    ...
                    if cognitive_load_handness:
                        f.write(str(pin_condition_order[i]) + "," + str(cognitive_load_handness) + "," + str(force_profile_electronic_component_pair[0]) + "," + str(force_profile_electronic_component_pair[1]) + "," +str(self.get_ask_last_num()))
                    else:
                        f.write(str(pin_condition_order[i]) + "," + str(cognitive_load_handness) + "," + str(force_profile_electronic_component_pair[0]) + "," + str(force_profile_electronic_component_pair[1]) + ","+str(-1))
                    f.write("\n")
    ...
    def read_command(self, filename=None):
        counter = 0
        if filename is None:
            filename = "User_Records/Participant"+str(self.current_user_num) + "/order_list.txt"
    ...
    def produce_random_list(self):
        ...
        # print "last_num_1: " + str(self.last_num.count(1))
        # print "last_num_2: " + str(self.last_num.count(2))
